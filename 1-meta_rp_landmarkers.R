...
metadatasets=c("original_RP.csv","pairwise_RP.csv","ranking_RP.csv","ratio_RP.csv")
metrics=c("rmse","nmae")
i <- 0
j <- 1
...
saveRDS(complete_results,"landmarkers_results_meta_rp.rds")
saveRDS(detailed_results, "landmarkers_detailed_results_rp.rds")
...
complete_results <- readRDS("landmarkers_results_meta_rp.rds")

i <- 1
new_metrics <- c("Accuracy_rmse","Accuracy_nmae")
metrics <- c("RMSE","NMAE")
metadatasets=c("AB","PW","RK","RT")
...
complete_results <- readRDS("landmarkers_results_meta_ir.rds")
i <- 1
