...
metadatasets=c("RM.csv","SL_IR.csv","GR.csv","CM_IR.csv")
metrics=c("ndcg","auc")
i <- 0
j <- 1
...
saveRDS(complete_results,"results/new_results_meta_ir.rds")
saveRDS(detailed_results, "results/new_detailed_results_ir.rds")
...
# complete_results <- readRDS("results/results_meta_ir.rds")
# 
# i <- 1
...
#   data2 <- data.frame(Algorithm=complete_results[[1]]$Algorithm)
...
#   
#   #assign baseline
#   baseline <- data2[which(data2[,1] == "majority vote"),2]
#   
#   #filter baseline
#   data2 <- data2[which(data2$Algorithm != "majority vote"),]
#   print(data2)
#   colnames(data2) <- c("Algorithm","C","E","D","B")
#   data2 <- data2[,c(1,5,2,4,3)]
#   
#   
#   data3 <- melt(data2)
#   print(data3)
...
#     coord_cartesian(ylim = c(min(data_baseline$value,data3$value)-0.01,max(data3$value))) +
#     geom_bar(stat = "identity", width = 1, position = "dodge") +
#     scale_colour_manual(values = c("black","red", "green")) +
#     geom_line(data=data_baseline, mapping=aes(x=Algorithm, y=value, group=variable, colour="Majority Voting"), size = 1) +
#     guides(colour = guide_legend(title = "Baseline"),
#            fill = guide_legend(title = "Meta-algorithms"))+
#     facet_grid(. ~ variable) +
